...
Stacking::Stacking(Dali::Application &application)
        : mApplication(application)
        , mTimer(Dali::Timer::New(1000 / 60))
		, mWorld()
        , mLeapData(std::make_shared<LeapData>()) {
    mApplication.InitSignal().Connect(this, &Stacking::Create);
    mApplication.TerminateSignal().Connect(this, &Stacking::Terminate);
    mTimer.TickSignal().Connect(this, &Stacking::Update);
    mAttempts = 1;
    mEOTest = false;
}

void Stacking::Create(Dali::Application &application) {
   ...
   for (int i = 0 ; i < 15 ; ++i){
	...
	mObjList.push_back(newobj = std::make_shared<BoxObject>(glm::vec3(-250.0f, -650.0f, 0.0f),
															0.0f,
															Vector3::ONE * 500.f));
	...
	//dynamic Objects
	mObjList.push_back(newobj = std::make_shared<BoxObject>(glm::vec3(-250.0f, -350.0f, 0.0f),
															1.0f,
															Vector3::ONE * 100.f));
	(*newobj).setMaterial("cube.obj.txt", "cube1.mtl")
			 .setRestitution(0.0f);

	mObjList.push_back(newobj = std::make_shared<BoxObject>(glm::vec3(-146.0f, -350.0f, 0.0f),
															1.0f,
															Vector3::ONE * 100.f));
	(*newobj).setMaterial("cube.obj.txt", "cube1.mtl")
			 .setRestitution(0.0f);

	mObjList.push_back(newobj = std::make_shared<BoxObject>(glm::vec3(-198.0f, -250.0f, 0.0f),
															1.0f,
															Vector3::ONE * 100.f));
	(*newobj).setMaterial("cube.obj.txt", "cube1.mtl")
			 .setRestitution(0.0f);

	//create docking locations
	mDockList.push_back(newobj = std::make_shared<BoxObject>(glm::vec3(150.0f, -300.0f, 0.0f),
															0.0f,
															Vector3::ONE * 200.f));
	(*newobj).setMaterial("cube.obj.txt", "cube.mtl")
			 .setRestitution(0.0f);

	mDockList.push_back(newobj = std::make_shared<BoxObject>(glm::vec3(150.0f, -125.0f, 0.0f),
															0.0f,
															Vector3::ONE * 150.f));
	(*newobj).setMaterial("cube.obj.txt", "cube.mtl")
			 .setRestitution(0.0f);

	mDockList.push_back(newobj = std::make_shared<BoxObject>(glm::vec3(150.0f, 0.0f, 0.0f),
															0.0f,
															Vector3::ONE * 100.f));
	(*newobj).setMaterial("cube.obj.txt", "cube.mtl")
			 .setRestitution(0.0f);
    ...
    for (auto &obj : mDockList)
    {
   		m3DLayer.Add(obj->getModel());
   		obj->getShape()->setSelected(true);
   		obj->update();
   	}
    ...
    stage.KeyEventSignal().Connect(this, &Stacking::OnKeyEvent);

    KuMedia::Receiver::Instance()->run();
    mTimer.Start();
}

void Stacking::Terminate(Dali::Application &application) {

}


bool Stacking::Update()
{
	...
	if(hand_detected)
	{
		for(int i = 0; i < mLeapData->mFrame.numHand; i++)			//loop for each hand recognized
		{
			...
			lVelocity = glm::vec3(0.0f, 0.0f, 0.0f);	//calculate the linear velocity
...
//				<< GestureInterpret(KuMedia::Receiver::Instance()->gesture_right) << "\n";
	...
	for (auto &obj : mObjList)
	{
		for (auto &dock : mDockList)
		{
			if((glm::abs(obj->getShape()->getSize() - dock->getShape()->getSize()) < 10) &&
			   (glm::abs(obj->getShape()->getWorldTransform()->getOrigin().x - dock->getShape()->getWorldTransform()->getOrigin().x) < 5) &&
			   (glm::abs(obj->getShape()->getWorldTransform()->getOrigin().y - dock->getShape()->getWorldTransform()->getOrigin().y) < 5) &&
			   (glm::abs(obj->getShape()->getWorldTransform()->getOrigin().z - dock->getShape()->getWorldTransform()->getOrigin().z) < 20))
			{
				obj->getShape()->getWorldTransform()->setOrigin(dock->getShape()->getWorldTransform()->getOrigin());
				obj->getShape()->getWorldTransform()->setBasis(dock->getShape()->getWorldTransform()->getBasis());

				obj->getShape()->setLinearVelocity(glm::vec3(0.0f, 0.0f, 0.0f));
				obj->getShape()->setAngularVelocity(glm::vec3(0.0f, 0.0f, 0.0f));

				float scale = dock->getShape()->getSize() / obj->getShape()->getSize();
				obj->getShape()->scale(scale);
				obj->getShape()->setScale(scale);

				obj->update();
				obj->getShape()->setScale(1.0f);
				count += 1;
			}
		}
		obj->update();
...
void Stacking::SetOffset(glm::vec3 leapPos)
{
	glm::vec3 scaleFactor = glm::vec3(1.25f, 3.0f, 1.0f);

	mOffset = leapPos * glm::vec3(1.0f, -1.0f, 1.0f);
	mOffset = mOffset + glm::vec3(0.0f, 260.0f, 100.0f);
	mOffset = mOffset * scaleFactor;
	mOffset = mOffset - (leapPos * glm::vec3(1.0f, -1.0f, 1.0f));
}

std::string Stacking::GestureInterpret(char c)
{
	if ((c - '0') == 0)
		return "default";
	if ((c - '0') == 1)
...
void Stacking::OnKeyEvent(const Dali::KeyEvent &event) {
    if (event.state == KeyEvent::Down) {
        ...
        if (IsKey(event, Dali::DALI_KEY_BACKSPACE))
        {
        	mFile << "Attempt " << mAttempts << ": \n";
			mFile << "Time: " <<  std::chrono::duration_cast<std::chrono::milliseconds>(Clock::now() - mAttemptTime).count() << "\n";
			mAttemptTime = Clock::now();
        	...
        	for (auto &obj : mObjList)
			{
				if(obj->getShape()->getInvMass() != 0.0f)
				{
					if(count == 0)
					{
						obj->getShape()->getWorldTransform()->setOrigin(glm::vec3(-250.0f, -350.0f, 0.0f));
					}
					if(count == 1)
					{
						obj->getShape()->getWorldTransform()->setOrigin(glm::vec3(-146.0f, -350.0f, 0.0f));
					}
					if(count == 2)
					{
						obj->getShape()->getWorldTransform()->setOrigin(glm::vec3(-198.0f, -250.0f, 0.0f));
					}

					obj->getShape()->getWorldTransform()->setBasis(mDockList[0]->getShape()->getWorldTransform()->getBasis());
					obj->getShape()->setLinearVelocity(glm::vec3(0.0f, 0.0f, 0.0f));
					obj->getShape()->setAngularVelocity(glm::vec3(0.0f, 0.0f, 0.0f));
					obj->getShape()->setSelected(false);

					float scale = 50.0f / obj->getShape()->getSize();
					obj->getShape()->scale(scale);
					obj->getShape()->setScale(scale);
		...
		{
        	mTotalTime = mAttemptTime = Clock::now();
		}
    }
}
